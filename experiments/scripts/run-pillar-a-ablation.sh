...
CONFIG=${1:-experiments/configs/a2-encoding-vs-threading.env}
...
GIT_COMMIT="$GIT_COMMIT" GIT_DIRTY="$GIT_DIRTY" CONFIG="$CONFIG" ARMS="$ARMS" \
...
metadata = {
    ...
    "parameters": {
        ...
        "repetitionsPerArm": int(env["REPETITIONS"]),
        # encoding:threads. The first and last reproduce a1's two arms.
        "arms": env["ARMS"].split(),
        "sink": env["SINK"],
...
record_run() {
  ...
  variant="${arm%%:*}"
  threads="${arm##*:}"
  tag="${variant}-t${threads}-${rep}"
  stdout_file="$RAW/$tag.out"
  time_file="$RAW/$tag.time"
  gc_file="$RAW/$tag.gc"

  printf '  %-16s rep %d/%d ... ' "$variant/${threads}t" "$rep" "$REPETITIONS"

  # `set +e` around the run: an arm that dies under the cap is a result, not a
  ...
  RUN_TAG="$tag" VARIANT="${variant}-t${threads}" REP="$rep" EXIT_CODE="$rc" \
...
for rep in $(seq 1 "$REPETITIONS"); do
  # The arm order reverses on even repetitions so no arm is always the one
  # meeting a machine that has been busy for several minutes.
  order="$ARMS"
  if [ "$((rep % 2))" -eq 0 ]; then
    order="$(echo "$ARMS" | tr ' ' '\n' | tail -r | tr '\n' ' ')"
  fi
...
say "summarising"
python3 "$ROOT/experiments/scripts/summarise-pillar-a-ablation.py" "$RAW"
